...
namespace HelperDock
{
    ...
    {
        // === injected: EnsureViewLoaded (safe logging only) ===
        private bool _ensureTried;

        internal void EnsureViewLoaded()
        {
            try
            {
                if (_ensureTried) { ModCompat.log.Info("[HelperDock] EnsureViewLoaded: already tried"); return; }
                _ensureTried = true;

                var t = this.GetType();
                ModCompat.log.Info("[HelperDock] EnsureViewLoaded: type=" + t.FullName);
                ...
                {
                    string n = m.Name;
                    if (n.IndexOf("View", System.StringComparison.OrdinalIgnoreCase) >= 0 &&
                        n.IndexOf("Binding", System.StringComparison.OrdinalIgnoreCase) < 0 &&
                        n.IndexOf("SaveLoad", System.StringComparison.OrdinalIgnoreCase) < 0)
                    {
                        cand++;
                        ModCompat.log.Info("[HelperDock] EnsureViewLoaded: candidate " + n + " (" + m.GetParameters().Length + " params)");
                    }
                }
                // scan base type (if any)
                var bt = t.BaseType;
                if (bt != null)
                {
                    foreach (var m2 in bt.GetMethods(flags))
                    {
                        string n = m2.Name;
                        if (n.IndexOf("View", System.StringComparison.OrdinalIgnoreCase) >= 0 &&
                            n.IndexOf("Binding", System.StringComparison.OrdinalIgnoreCase) < 0 &&
                        ...
                        {
                            cand++;
                            ModCompat.log.Info("[HelperDock] EnsureViewLoaded: candidate base." + n + " (" + m2.GetParameters().Length + " params)");
                        }
                    }
                }

                if (cand == 0)
                    ModCompat.log.Info("[HelperDock] EnsureViewLoaded: no candidates");
                // NOTE: open call is not executed yet; we first verify candidates/logging path.
            }
            catch (System.Exception ex)
            {
                ModCompat.log.Info("[HelperDock] EnsureViewLoaded exception: " + ex.Message);
            }
        }
        // === /injected ===

        private int _tick = 0;
        private bool TickGate(int interval = 6)
        {
            this._tick++;
            return (this._tick % interval) == 0;
        }
        // ======================================
        ...
        {
      IMGUIBlocker.RunOnce(); UnityEngine.Debug.Log("[HelperDock] UISystem.OnCreate");
            base.OnCreate();

            try
            {
                ModCompat.log.Info($"{nameof(UISystem)}.{nameof(OnCreate)}");
                ...
                AddBinding(_bindingMainPanelVisible   = new ValueBinding<bool  >(UIEventName.GroupName, UIEventName.MainPanelVisible,   HelperDock.ModCompat.ModSettings.MainPanelVisible  ));
                AddBinding(_bindingMainPanelPositionX = new ValueBinding<int   >(UIEventName.GroupName, UIEventName.MainPanelPositionX, HelperDock.ModCompat.ModSettings.MainPanelPositionX));
                AddBinding(_bindingMainPanelPositionY = new ValueBinding<int   >(UIEventName.GroupName, UIEventName.MainPanelPositionY, HelperDock.ModCompat.ModSettings.MainPanelPositionY));
                ...
                AddBinding(new ValueBinding<bool>(UIEventName.GroupName, UIEventName.ShowGPUUsage,    ModCompat.ShowGPUUsage   ));
                AddBinding(new ValueBinding<bool>(UIEventName.GroupName, UIEventName.ShowCPUUsage,    ModCompat.ShowCPUUsage   ));
                AddBinding(new ValueBinding<bool>(UIEventName.GroupName, UIEventName.ShowMemoryUsage, ModCompat.ShowMemoryUsage));
            }
            catch(Exception ex)
            {
                ModCompat.log.Error(ex);
            }
        }
        ...
        protected override void OnGameLoadingComplete(Purpose purpose, GameMode mode)
        {
            ...
            // ensure main panel visible after initialize
            EnsureMainPanelVisibleOnce();
}

        /// <summary>
        /// Initialize this mod to get everything ready for a newly started game.
        /// </summary>
        // Ensure main panel is visible once after Initialize (PerformanceMonitor style)
        private bool _panelShownOnce = false;
        private void EnsureMainPanelVisibleOnce()
        {
            // === guard: do not auto-show on main menu ===
            if (!_inGame) {
                UnityEngine.Debug.Log("[HelperDock] EnsureMainPanelVisibleOnce: skip (not in-game)");
                return;
            }
            if (_panelShownOnce) return;
            try
            {
                // already visible?
                if (_bindingMainPanelVisible != null && _bindingMainPanelVisible.value == true)
                {
                    _panelShownOnce = true;
                    ModCompat.log.Info("[HelperDock] EnsureMainPanelVisibleOnce: already visible");
                    return;
                }

                // binding + settings → true（保存まで）
                if (_bindingMainPanelVisible != null)
                {
                    _bindingMainPanelVisible.Update(true);
                }
                HelperDock.ModCompat.ModSettings.MainPanelVisible = true;
                HelperDock.ModCompat.ModSettings.ApplyAndSave();

                UnityEngine.Debug.Log("[HelperDock] EnsureMainPanelVisibleOnce: ON");
                _panelShownOnce = true;
            }
            catch (System.Exception ex)
            {
        ...
        private void Initialize()
        {
            try
            {
                ModCompat.log.Info($"{nameof(UISystem)}.{nameof(Initialize)}");
...
ProxyAction activationKeyAction = HelperDock.ModCompat.ModSettings.GetAction(HelperDock.ModCompat.Settings.ActivationKeyActionName);
...
UnityEngine.Debug.Log("[HelperDock] Init check: visBinding=" + (_bindingMainPanelVisible!=null) + ", posX=" + (_bindingMainPanelPositionX!=null) + ", posY=" + (_bindingMainPanelPositionY!=null) + ", settings.Visible=" + HelperDock.ModCompat.ModSettings.MainPanelVisible);

            // DockOverlayMono.Ensure(); // (disabled) // disabled (cohtml only)
}
        ...
        private void Deinitialize()
        {
            try
            {
                ModCompat.log.Info($"{nameof(UISystem)}.{nameof(Deinitialize)}");
...
base.OnUpdate();
            ...
            if (!_inGame)
            {
                ...
                if (TickGate(120)) UnityEngine.Debug.Log("[HelperDock] UISystem.OnUpdate (inGame)");
                    ...
                    if (simulationSpeed > 0 && !_savingGame)
                    {
                        ...
                        else
                        {
                            ...
                            if (_simTimingChangedOnce)
                            {
                                ...
                                if (HelperDock.ModCompat.ModSettings.MainPanelVisible)
                                {
        ...
        /// <summary>
        /// Initialize CPU usage.
        /// This is a separate function to avoid a reference to the Windows-only ModCompat.CPUUsage class in the Initialize and OnUpdate functions.
        /// </summary>
        private void InitializeCPUUsage()
        {
            // If ModCompat.CPUUsage fails to initialize, then do not show CPU usage.
            ModCompat.ShowCPUUsage = CPUUsage.Initialize();
        }

        /// <summary>
        /// Display GPU usage value.
        /// This is a separate function to avoid a reference to the Windows-only ModCompat.GPUUsage class in the OnUpdate function.
        /// </summary>
        ...
        /// This is a separate function to avoid a reference to the Windows-only ModCompat.CPUUsage class in the OnUpdate function.
        /// </summary>
        ...
        /// This is a separate function to avoid a reference to the Windows-only ModCompat.MemoryUsage class in the OnUpdate function.
        /// </summary>
        ...
        private void MainButtonClicked()
        {
            ...
            HelperDock.ModCompat.ModSettings.MainPanelVisible = newVisible;
            HelperDock.ModCompat.ModSettings.ApplyAndSave();
        ...
        private void MainPanelMoved(int positionX, int positionY)
        {
            ...
            HelperDock.ModCompat.ModSettings.MainPanelPositionX = positionX;
            HelperDock.ModCompat.ModSettings.MainPanelPositionY = positionY;
            HelperDock.ModCompat.ModSettings.ApplyAndSave();
        ...
        // === injected: hotkey helpers ===
        internal void ToggleMainPanel()
        {
            if (_bindingMainPanelVisible == null)
            {
                UnityEngine.Debug.Log("[HelperDock] ToggleMainPanel: visible binding is null");
                return;
            }
            bool next = !(HelperDock.ModCompat.ModSettings?.MainPanelVisible ?? false);
            _bindingMainPanelVisible.Update(next);
            HelperDock.ModCompat.ModSettings.MainPanelVisible = next;
            HelperDock.ModCompat.ModSettings.ApplyAndSave();
            UnityEngine.Debug.Log("[HelperDock] ToggleMainPanel => " + next);
        }

        internal void ForceShowNow()
        {
            if (_bindingMainPanelVisible == null)
            {
                UnityEngine.Debug.Log("[HelperDock] ForceShowNow: visible binding is null");
                return;
            }
            _bindingMainPanelPositionX?.Update(80);
            _bindingMainPanelPositionY?.Update(120);
            HelperDock.ModCompat.ModSettings.MainPanelPositionX = 80;
            HelperDock.ModCompat.ModSettings.MainPanelPositionY = 120;
            _bindingMainPanelVisible.Update(true);
            HelperDock.ModCompat.ModSettings.MainPanelVisible = true;
            HelperDock.ModCompat.ModSettings.ApplyAndSave();
            UnityEngine.Debug.Log("[HelperDock] ForceShowNow: Update(true) @ (80,120)");
        }
        // === /injected ===
    }
}
